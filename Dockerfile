...
#FROM 812206152185.dkr.ecr.us-west-2.amazonaws.com/latch-base:dd8f-main as latch_build
FROM 812206152185.dkr.ecr.us-west-2.amazonaws.com/latch-base-cuda:ca95-main as latch_build
...
WORKDIR /opt/
RUN git clone https://github.com/google/deepvariant.git && cd deepvariant && git checkout r1.4
...
FROM latch_build as builder
COPY --from=conda_setup /opt/conda /opt/conda
COPY --from=latch_build /opt/deepvariant /opt/deepvariant

ARG DV_GPU_BUILD
ENV DV_GPU_BUILD=${DV_GPU_BUILD}

# Copying DeepVariant source code
#COPY ./deepvariant /opt/deepvariant
...
RUN sed -i 's/"${CLIF_PIN-}"/"fbc9fcf"/g' ./tools/build_clif.sh
RUN ./build-prereq.sh 
...
RUN python3 -m pip install 'protobuf==3.13.0'
